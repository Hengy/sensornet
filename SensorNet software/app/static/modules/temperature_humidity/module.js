...
function load_current_values() {
	...
	var tempValue = currentModuleInfo.values[0][1];
	$("#current_module_temp").append(tempValue.toFixed(1) + '<div class="current_module_unit">&deg;C</div>');
	
	var humidValue = currentModuleInfo.values[0][0];
	$("#current_module_humidity").append(humidValue.toFixed(1) + '<div class="current_module_unit">%</div>');
}
